...
class DataFlowJsonTestCase(BaseTestCase):
    def setUp(self):
        super().setUp()
    ...
    def __memory(self) -> DataFlow.DataFrame:
        return DataFlow().DataFrame().from_json(self.TEST_JSON_FILE)

    def __parquet(self) -> DataFlow.DataFrame:
        return DataFlow().DataFrame(in_memory=False).from_json(self.TEST_JSON_FILE)

    def __feather(self) -> DataFlow.DataFrame:
        return DataFlow().DataFrame(in_memory=False, file_type=FileType.feather).from_json(self.TEST_JSON_FILE)
